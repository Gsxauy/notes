...
``` js
  class User {
    ...
    show() {} // 打印发现  这个方法已经自动放置在 原想prototype 上面去了
  }
...
* 添加到函数原型到方法 (User.prototype.show = function() {}) 如果没有设置具体特征(枚举)的时候是可以 `in` 遍历出来的
* 如果指向当前遍历的 那就是 Object.hasOwnProperty(key); 
* 使用**类**的时候 会自动添加 不可枚举的 特性。
...
``` js
  class User {
    ...
    static create(...args) { // 展开语法 可以接受 不定数量的参数
      return new this(...args) // new this 是因为 this 就是声明 当前的 类
    }
  }
  let gl = User.create('郭霖', 19); // 静态方法的调用
  console.log(gl) // User {name: "郭霖"} 
```
...
``` js
  ...
  class Lesson {
    ...
    // 静态方法 就是我们不是对 单个对象进行管理的 这样使用静态方法就比较了
    ...
    static totalPrice(data) { // 如果不是对单个对象操作的 就可以使用静态方法
      return data.reduce((t, c) => {
        return t + c.price()
      }, 0)
    }
  }
  let obj1 = mew Lesson(data);
  console.log(obj1);
  let Lessons = Lesson.createBatch(data); // 课程的集合
  console.log('Lessons', Lessons);
  console.log(Lesson.maxPrice(Lessons).name()); // js 这样是可以调用的 因为也是对象的方法 this.name()
  console.log(Lesson.maxPrice(Lessons).name1); // js 如果改成访问器的名字 就可以不需要写括号了 本质上当作属性访问
  console.log(Lesson.totalPrice(Lessons)); // FIXME:  如果是对单个对象操作的话 就不能使用静态方法了
```
...
* 唯一值 受保护的属性 不让呗设置的属性 可以使用 Symbol
...
* `super` 关键字解决了 多层调用的问题 上面的this 还是打印的 `gg`
...
let gg = {
  ...
  show() {
    ...
    this.__proto__.show.call(this) // gg.name 这里就改变了 this 之后 打印的 类似是实现了 super的操作机制
    super.show() // gg.name
  }
...
* 扩展 子类调用 父类的方法 来扩展自己的方法 完成重写 `super.xx()`
...
* 静态方法是 通用 的里面不能有对象数据 如果有对象数据 那就不通用了
...
``` js
  	function checkPrototype(obj, cn) {
  	  obj = Object.getPrototypeOf(obj)
  	  if (!obj) return false;
  	  if (obj === con.prototype) return true
  	  return checkPrototype(obj, con);
  	}
...
``` js
// a.isPrototypeOf(b)  b对象是否是由a对象实现的
class User {}
class Admin extends User {}
let ad = new Admin();
