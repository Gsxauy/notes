...
let obj = { // 对象 转换字符串 需要指定索引和 length 属性 然后 Array.from 就可以对其进行转换了~
  0 : 'b',
  length: 2
}
console.log(Array.from(obj));
// Array.from 的 第二个参数就是一个 回到函数-作用就是 对其中的每个元素进行遍历~
```
...
[...args].forEach || [...div].map // 这样就转换成数组了 - 可以使用数组的方法了~
```
...
// - 从第一个位置添加 添加到第三个位置 (包括起始 不包括结束) - 返回新的数组 - 会修改原数组
...
+ `引用类型` 去做 循环赋值操作 -  就会改变原来数组里面的对象 - 因为使用的是同一个引用地址~
  ...
  ```js
  let arr = [1, 5, 3, 8];
  for (const item of arr) { // 循环 value
    console.log(item); // 1, 5, 3, 8
  }
  for (const key in arr) { // 循环 key
...
for (const key in arr.values()) { // Array Iterator {} - 因为 `arr.values()` 返回的是一个 迭代对象
...
let b = a.map((item) => {
  ...
  return { age: 18, ...item } // 这样也可以 - 反正都是浅拷贝
...
const div = document.querySelector('div');
  // console.log([...div.textContent]); // ["V", "x", "e", "c", "h", "o"]
  [...div.textContent].reduce((pre, cur, index) => {
    ...
    span.addEventListener('animationend', function () { // 这个是监听动画结束
